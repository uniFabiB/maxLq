...
for [i=1:words(FILES):1] \
	word(FILES,i) u (g($2)):(f($5)) title word(FILES,i)." tau-a" with linespoints ls i pointtype 2, \
for [i=1:words(FILES):1] \
	word(FILES,i) u (g($3)):(f($6)) title word(FILES,i)." tau-b" with linespoints ls i pointtype 4, \
for [i=1:words(FILES):1] \
	word(FILES,i) u (g($4)):(f($7)) title word(FILES,i)." tau-c" with linespoints ls i pointtype 6
